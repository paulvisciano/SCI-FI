...
function animateNeurograph() {
  ...
  if (neurographScene && neurons.length > 0 && isNeurographLoaded) {
    ...
    // Gentle fade in/out effect for neurons - subtle breathing (4.2s cycle)
    const baseOpacity = 0.7 + 0.2 * Math.sin(Date.now() * 0.0015);
    neurons.forEach((neuron, idx) => {
      if (neuron.material && neuron.material.opacity !== undefined) {
        neuron.material.opacity = baseOpacity;

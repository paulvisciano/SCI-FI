...
function isValidInput(input) {
  // Check length
  if (typeof input !== 'string' || input.length > MAX_MESSAGE_LENGTH) {
    return false;
  }
...
function handleRequest(req, res) {
  ...
  if (req.method === 'GET' && req.url === '/api/vitals') {
    ...
    checkOllamaHealth().then(ollamaHealth => {
      ...
      try {
        ...
        // Memory info - use os module for system-wide totals (all processes, entire computer)
        try {
          ...
          vitals.system.memory.usedPercent = usedPercent;
        } catch (err) {
        ...
        // Disk usage via df -h /
        try {
          const dfResult = spawnSync('df', ['-h', '/'], { encoding: 'utf8' });
          const dfLines = dfResult.stdout.trim().split('\n');
          if (dfLines.length >= 2) {
            const dfParts = dfLines[1].split(/\s+/);
            // df -h / output: Filesystem Size Used Avail Capacity iused ifree %iused Mounted on
            // Example: /dev/disk3s1s1 466Gi 285Gi 181Gi 62% 2940097 2330053613 0% /
            const sizeStr = dfParts[1];
            const usedStr = dfParts[2];
            const capacityPercent = parseInt(dfParts[4].replace('%', ''), 10);
              
            vitals.system.disk = {
              total: sizeStr,
              used: usedStr,
              usedPercent: capacityPercent || 0
            };
          }
        } catch (err) {
          console.log('Could not get disk info:', err.message);
        }
      } catch (err) {
      ...
      http.get(`http://localhost:${ollamaPort}/api/tags`, (res) => {
        ...
        res.on('end', () => {
          try {
            ...
            // Extract model info: name, size, type (local/remote), parameter size
            const modelList = (models.models || []).map(m => ({
              name: m.name,
              size: m.size,
              isRemote: !!m.remote_model,
            ...
            resolve({ status: 'Connected', models: 0, modelList: [], recentErrors: 0, error: null });
          }
        });
      }).on('error', (err) => {
        resolve({ status: 'Disconnected', models: 0, modelList: [], recentErrors: 0, error: err.message });
      });
    });
  }
  ...
  if (req.method === 'POST' && req.url === '/upload') {
    ...
    req.on('end', () => {
      ...
      // Security: Check upload size limit
      if (buffer.length > MAX_FILE_SIZE) {
        console.error(`[SECURITY] Rejected upload (size: ${buffer.length} bytes > ${MAX_FILE_SIZE} bytes)`);
        res.writeHead(413, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ ok: false, error: 'File too large', maxBytes: MAX_FILE_SIZE }));
        return;
      }
      ...
      // Security: Check parsed audio data size as well
      if (audioData.length > MAX_FILE_SIZE) {
        console.error(`[SECURITY] Rejected audio data (size: ${audioData.length} bytes > ${MAX_FILE_SIZE} bytes)`);
        res.writeHead(413, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ ok: false, error: 'File too large', maxBytes: MAX_FILE_SIZE }));
        return;
      }
  ...
  if (req.method === 'POST' && req.url === '/api/process-inbox') {
    ...
    try {
      ...
      // Security validation for user message
      if (!isValidInput(userMessage)) {
        console.error(`[SECURITY] Rejected message (length: ${userMessage.length}, chars > 5000 or contains dangerous chars)`);
        res.writeHead(400, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ status: 'error', message: 'Invalid input' }));
        return;
      }
            
      // Run openclaw agent asynchronously (non-blocking) with timeout
      const timestamp = new Date().toISOString();
      const agentStart = Date.now();
      console.log(`[${timestamp}] ⏱️ Agent START`);
            
      // Use spawn with array format for security
      const agentProcess = spawn('openclaw', ['agent', '--agent', 'jarvis', '--message', userMessage], { encoding: 'utf8' });
            
      // Agent timeout
      const agentTimeout = setTimeout(() => {
        console.error(`[${timestamp}] ⏱️ Agent TIMEOUT (>15s)`);
        if (agentProcess.connected) {
          agentProcess.kill('SIGKILL');
        }
        res.writeHead(500, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ status: 'error', message: 'Agent timeout (>15s)' }));
      }, AGENT_TIMEOUT);
      ...
      agentProcess.on('close', code => {
        ...
        if (code !== 0) {
          console.error(`[${agentTimestamp}] ❌ Agent failed (exit code: ${code})`);
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ status: 'error', message: `Agent exited with code ${code}` }));
          return;
        }
        ...
        // Post to Jarvis agent session (OpenClaw) with timeout
        const sessionTimeout = setTimeout(() => {
          console.error(`[${agentTimestamp}] ⏱️ Session send TIMEOUT (>15s)`);
        }, AGENT_TIMEOUT);
                
        const sessionProcess = spawn('openclaw', ['sessions', 'send', '--sessionKey', 'agent:jarvis:main', '--message', responseText]);
                
        sessionProcess.on('close', sessionCode => {
          clearTimeout(sessionTimeout);
          if (sessionCode !== 0) {
            console.error(`[${agentTimestamp}] ⚠️ Session send failed (exit code: ${sessionCode})`);
          } else {
      ...
      agentProcess.on('error', (error) => {
        clearTimeout(agentTimeout);
        console.error(`[${timestamp}] ❌ Agent process error:`, error.message);
        res.writeHead(500, { 'Content-Type': 'application/json' });
...
function processRecording(filepath, extension) {
  ...
  if (extension === '.webm') {
    ...
    // Use spawn with timeout for safety
    const ffmpegProcess = spawn(ffmpegPath, ['-i', filepath, '-ar', '16000', '-ac', '1', wavPath, '-y'], { encoding: 'utf8' });
        
    const ffmpegTimeout = setTimeout(() => {
      console.error(`[${timestamp}] ⏱️ FFmpeg TIMEOUT (>60s)`);
      if (ffmpegProcess.connected) {
        ffmpegProcess.kill('SIGKILL');
      }
      currentTranscription = { status: 'error', error: 'FFmpeg timeout' };
      lastError = { 
        type: 'ffmpeg', 
        message: 'FFmpeg timeout (>60s)', 
    ...
    ffmpegProcess.on('close', code => {
      ...
      if (code !== 0) {
        const errMsg = '❌ FFmpeg conversion failed (exit code: ' + code + ')';
    ...
    ffmpegProcess.on('error', (error) => {
      clearTimeout(ffmpegTimeout);
      console.error(`[${new Date().toISOString()}] ❌ FFmpeg process error:`, error.message);
      currentTranscription = { status: 'error', error: 'FFmpeg process error' };
      lastError = { 
        type: 'ffmpeg', 
...
function transcribeWithWhisper(audioPath, modelPath, extension) {
  const timestamp = new Date().toISOString();
  // Use spawn with array format for security
  const whisperProcess = spawn(CONFIG.whisperCli, ['-m', modelPath, '-otxt', audioPath], { encoding: 'utf8' });
    
  // Timeout for transcription (max 60 seconds)
  const whisperTimeout = setTimeout(() => {
    console.error(`[${timestamp}] ⏱️ Whisper TIMEOUT (>60s)`);
    if (whisperProcess.connected) {
  ...
  whisperProcess.on('close', code => {
    ...
    setTimeout(() => {
      if (fs.existsSync(txtFile)) {
        ...
        console.error(`[${timestamp}] ❌ No transcript created (exit code: ${code})`);
  ...
  whisperProcess.on('error', (error) => {
    clearTimeout(whisperTimeout);
    console.error(`[${timestamp}] ❌ Whisper process error:`, error.message);
    currentTranscription = { status: 'error', error: 'Whisper process failed' };
    activeTranscriptions--;
  });
}
...
function handleTranscript(filepath, transcript, extension) {
  ...
  // Use spawn with array format for security (prevents command injection)
  const agentProcess = spawn('openclaw', ['agent', '--agent', 'jarvis', '--message', userMessage], { encoding: 'utf8' });
    
  // Agent timeout
  const agentTimeout = setTimeout(() => {
    console.error(`[${new Date().toISOString()}] ⏱️ Agent TIMEOUT (>15s)`);
    if (agentProcess.connected) {
      agentProcess.kill('SIGKILL');
    }
  }, AGENT_TIMEOUT);
  ...
  agentProcess.on('error', (error) => {
    clearTimeout(agentTimeout);
    console.error(`[${new Date().toISOString()}] ❌ Agent process error:`, error.message);
    // Still archive on error
    archiveRecording(filepath, extension, transcript);
    activeTranscriptions--;
  });
}
